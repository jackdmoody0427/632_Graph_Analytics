...
;  show "checking if connected"
  ...
  report (count giant-component = count turtles)
end
...
to rewire
    ...
    ask one-of links [
      ...
      set n3 one-of turtles with [(not link-neighbor? n1) and (self != n1)]
      set d1 (count [link-neighbors] of n1)
      set d2 (count [link-neighbors] of n2)
      set d3 (count [link-neighbors] of n3)

      if ((assortativity * (abs(d1 - d2) - abs(d1 - d3)) > 0) and (d2 > 1))[
        ask n1 [create-link-with n3 [set new-link self]]
        die
      ]
    ]

    set is-connected do-calculations
    if ((not is-connected) and (new-link != nobody)) [
...
to attack
  ask max-n-of num-remove turtles [count link-neighbors] [ die ]
  set is-connected do-calculations
end
...
A model to test robustness of assortative and disassortative networks.

## HOW TO USE IT

The NUM-NODES slider controls the size of the network.

The PROB-PREF parameter determines whether the attachment is preferential. With probability PROB-PREF, the node will chose to attach to existing nodes in proportion to their degree. With probability (1 - PROB-PREF) it will attach to any existing node with equal probability.

Choose num-nodes and PROB-PREF and press SETUP.

The network will first be setup using a growth process, and then will rewire for a while to become assortative (ASSORTATIVITY = 1 from the dropdown menu), or disassortative (ASSORTATIVITY = 10)

Once it's done, you can do a targeted ATTACK, taking out the NUM-REMOVE highest degree nodes.

## THINGS TO NOTICE

How does the assorativity affect how resilient the network is to targeted attack?

## CREDITS AND REFERENCES

Copyright 2012 Lada Adamic, adapted from the NetLogo models library written by Uri Wilensky.
@#$#@#$#@
default
true
0
